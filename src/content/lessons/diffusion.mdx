...
<div class="glossary">
<label><input type="checkbox" checked /> Keep visible</label>
...
We can also express the distribution at step ~t-1~ as a function of the position at step ~t~ and the initial data point ~x_0~.
...
<T block v='
forall t in [1..T], q(x_(t-1) | x_t, x_0) & prop cal(N)(tilde(mu), tilde(beta)^2 I) \
"with" \
...
## The learnable backward process
...
## From a global to a local objective

We will now show that we can simplify the global objective into a sum of local objectives, one for each step, with the proper conditioning.
...
- decomposing the KL divergence as a sum of KL divergences between each step, being careful on the writting, expeactations and conditionings,
- using the "sandwich view" to "reverse" the forward noising process.
...
TODO not sure how to deal with log and EE
...
Fig. <Ref label="fig:all-together"/> summarizes all the pieces together.

<figure>
    <InlineSvg asset="diffusion" hide='#more, #backward' />"KL"(q(x_(0:T)) || p_theta(x_(0:T)))
