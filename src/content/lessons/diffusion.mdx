...
<details>
<summary>The case of ~t=1~ (empty sandwich)</summary>
<div>
...
To avoid having handling this special case below, we override ~tilde(mu)(x_0, t=0, x_t) := 1 / sqrt(1 - beta_1) x_t~ and ~tilde(beta)(t=0) := beta_1 / (1 - beta_1)~.
</div>
</details>
...
&<=&& sum_(t=1)^T lambda_t EE_(x_t tilde.op q(x_t)) EE_(x_0 tilde.op q(x_0)) [(tilde(mu)(x_0, t, x_t) - mu_theta (x_t, t))^2] + C \
&<=&& sum_(t=1)^T lambda_t EE_(x_0 tilde.op q(x_0)) EE_(x_t tilde.op q(x_t|x_0)) [(tilde(mu)(x_0, t, x_t) - mu_theta (x_t, t))^2] + C \
'/>

with ~lambda_t = 1/(2 sigma_t^2)~.
The last line obtained by replacing the independent sampling of ~x_0~ and ~x_t~ with a conditional sampling of ~x_t | x_0~.
